"""
apps.analytics.services.offline_eval_service 单元测试 —— 离线评估 Pipeline（黄金测试集 + 检索/回答评估）
...
- generate_answer：LLM 响应组装与异常降级

说明：检索与 LLM 全部在源模块导入处 mock（apps.retrieval.hybrid.hybrid_search、
apps.llm.factory.get_llm、apps.analytics.services.deepeval_service.evaluate_with_deepeval 等），
不依赖真实向量库与外部模型。
"""
import pytest
from unittest.mock import patch, MagicMock

from apps.analytics.services import offline_eval_service
from apps.users.models import User
from apps.knowledge.models import KnowledgeNode, Document
from apps.analytics.models import (
    GoldenQuestion, GoldenRelevantDoc, GoldenReferenceAnswer,
)
...
class TestRetrievalMetrics:
    ...
    def test_recall_at_k_basic(self):
        ...
        relevant = {1, 2}
        retrieved = [1, 9, 2, 3]
        assert offline_eval_service._calc_recall_at_k(relevant, retrieved, k=5) == 1.0
        assert offline_eval_service._calc_recall_at_k(relevant, retrieved, k=1) == 0.5

    @pytest.mark.unit
    def test_recall_at_k_empty_relevant(self):
        """无相关文档标注 → 0（不参与分母为 0 的除零）"""
        assert offline_eval_service._calc_recall_at_k(set(), [1, 2], k=5) == 0.0
    ...
    def test_recall_at_k_k_greater_than_retrieved(self):
        ...
        relevant = {1, 2}
        retrieved = [1]
        assert offline_eval_service._calc_recall_at_k(relevant, retrieved, k=20) == 0.5

    @pytest.mark.unit
    def test_mrr_basic(self):
        """MRR = 1 / 首个相关文档的排名"""
        assert offline_eval_service._calc_mrr({3}, [1, 2, 3, 4]) == pytest.approx(1 / 3)
        assert offline_eval_service._calc_mrr({1}, [1, 2]) == 1.0

    @pytest.mark.unit
    def test_mrr_no_hit(self):
        """无相关命中 → 0"""
        assert offline_eval_service._calc_mrr({9}, [1, 2, 3]) == 0.0
        assert offline_eval_service._calc_mrr({9}, []) == 0.0
    ...
    def test_ndcg_perfect_ranking(self):
        ...
        relevant = {1, 2}
        retrieved = [1, 2, 3, 4]
        assert offline_eval_service._calc_ndcg_at_k(relevant, retrieved, k=5) == pytest.approx(1.0)

    @pytest.mark.unit
    def test_ndcg_no_hit(self):
        """无相关命中 → 0.0"""
        relevant = {9}
        assert offline_eval_service._calc_ndcg_at_k(relevant, [1, 2], k=5) == 0.0

    @pytest.mark.unit
    def test_ndcg_empty_relevant(self):
        """空相关文档集合 → 0.0"""
        assert offline_eval_service._calc_ndcg_at_k(set(), [1, 2], k=5) == 0.0
    ...
    def test_ndcg_partial_ranking_between_zero_and_one(self):
        ...
        relevant = {1, 2}
        retrieved = [9, 1, 2]
        ndcg = offline_eval_service._calc_ndcg_at_k(relevant, retrieved, k=5)
        assert 0 < ndcg < 1

    @pytest.mark.unit
    def test_avg(self):
        """平均值：空列表 0，正常列表求均值"""
        assert offline_eval_service._avg([]) == 0.0
        assert offline_eval_service._avg([1.0, 2.0, 3.0]) == 2.0
...
class TestGoldenDatasetManagement:
    ...
    def test_create_golden_dataset(self):
        """创建测试集：字段与默认值正确"""
        ds = offline_eval_service.create_golden_dataset(
    ...
    def test_import_questions_creates_records(self):
        ...
        ds = offline_eval_service.create_golden_dataset(name='ds', created_by_id=self.user.id)
        result = offline_eval_service.import_questions_from_json(ds.id, [
    ...
    def test_import_questions_updates_existing(self):
        ...
        ds = offline_eval_service.create_golden_dataset(name='ds')
        first = offline_eval_service.import_questions_from_json(ds.id, [
            {'question': 'Q', 'relevant_doc_ids': [self.docs[0].id],
             'reference_answer': 'a'},
        ])
        second = offline_eval_service.import_questions_from_json(ds.id, [
    ...
    def test_import_dataset_not_found(self):
        ...
        with pytest.raises(ValueError, match='not found'):
            offline_eval_service.import_questions_from_json(99999, [])

    def test_export_round_trip(self):
        """导出与导入字段往返一致；无参考答案时兜底为空"""
        ds = offline_eval_service.create_golden_dataset(name='ds')
        offline_eval_service.import_questions_from_json(ds.id, [
            {'question': 'Q1', 'question_type': 'reasoning', 'difficulty': 'hard',
             'tags': ['T'], 'relevant_doc_ids': [self.docs[0].id],
             'reference_answer': 'R', 'key_points': ['P']},
            {'question': 'Q2'},  # 无参考答案
        ])
        exported = offline_eval_service.export_dataset_to_json(ds.id)
...
class TestRunRetrievalEvaluation:
    ...
    def _env(self):
        ...
        self.dataset = offline_eval_service.create_golden_dataset(name='检索集', root_type='hr')
    ...
    def test_report_metrics_with_hits(self):
        ...
        with patch('apps.retrieval.hybrid.hybrid_search', return_value=search_result):
            report = offline_eval_service.run_retrieval_evaluation(self.dataset.id, user=self.user)
    ...
    def test_no_questions_raises(self):
        ...
        with pytest.raises(ValueError, match='no questions'):
            offline_eval_service.run_retrieval_evaluation(self.dataset.id)
...
class TestRunAnswerQualityEvaluation:
    ...
    def _env(self):
        ...
        self.node = KnowledgeNode.objects.create(
            ...
            created_by=self.user)
        self.dataset = offline_eval_service.create_golden_dataset(name='回答集', root_type='hr')
    ...
    def test_happy_path_results(self):
        ...
        with patch('apps.retrieval.hybrid.hybrid_search', return_value=search_result), \
             patch('apps.analytics.services.offline_eval_service.generate_answer', return_value='测试回答'), \
             patch('apps.analytics.services.deepeval_service.evaluate_with_deepeval',
                   return_value=eval_results):
            results = offline_eval_service.run_answer_quality_evaluation(self.dataset.id, user=self.user)
    ...
    def test_search_failure_records_error(self):
        ...
        with patch('apps.retrieval.hybrid.hybrid_search', side_effect=flaky), \
             patch('apps.analytics.services.offline_eval_service.generate_answer', return_value='a'), \
             patch('apps.analytics.services.deepeval_service.evaluate_with_deepeval',
...
class TestGenerateAnswer:
    ...
    def test_generate_answer_success(self):
        """正常路径：调用项目 Provider 统一 chat() 接口并返回内容"""
        fake_llm = MagicMock()
        fake_llm.chat.return_value = {'content': '生成答案'}
        with patch('apps.llm.factory.get_llm', return_value=fake_llm):
            answer = offline_eval_service.generate_answer('问题', '上下文', 'deepseek-chat')
        assert answer == '生成答案'
