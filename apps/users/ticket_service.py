...
# ============================================================================
# 类型前缀（两字母大写拼音首字母）：权限 QX / 配置 PZ / 定时 DS / 模型 MX / 安全 AQ
# 示例：QX202608080001（当日第 1 单，权限）、AQ202608080002（当日第 2 单，安全配置）
TICKET_TYPE_PREFIX = {
...
# ============================================================================
# 安全配置工单服务（IP 白名单/黑名单/敏感词变更）
# ============================================================================

# 安全配置风险分级策略：
# - 低风险（直接生效）：黑名单新增、敏感词新增
# - 中风险（单审：compliance_admin 审核）：黑名单解封、敏感词删除/禁用
# - 高风险（双审：compliance_admin 审核 + super_admin 复核）：白名单新增/删除/编辑
SECURITY_RISK_LEVEL = {
...
def _build_security_approval_chain(risk_level: str) -> list:
    ...
    - low：空链（直接生效）
    - normal：单审（compliance_admin 审核）
    - high：双审（compliance_admin 审核 + super_admin 复核）
    """
    if risk_level == 'low':
        return []
