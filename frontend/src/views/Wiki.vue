...
function mdInline(text) {
  return text
    ...
    .replace(/\[([^\]]+)\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g, (m, text, url) => {
      ...
      // 站内 / 占位链接（# 或相对路径）：视为正文里的参考资料链接，
      // 点击由 onMdLinkClick 委托打开对应文档预览，而非新开页面
      if (!/^(https?:\/\/|mailto:)/i.test(safeUrl)) {
        return `<a class="wiki-md-ref" href="#" role="button">${mdInline(text)}</a>`
      }
      // 外部链接：新开页面（safeLink 已做协议白名单，防 javascript: 等危险协议）
      return `<a href="${escapeHtml(safeUrl)}" target="_blank" rel="noopener noreferrer">${mdInline(text)}</a>`
    })
    .replace(/\*\*([^*\n]+)\*\*/g, '<strong>$1</strong>')
...
function renderMarkdown(src) {
  ...
  // 行内代码：不在块级提前替换，改为各输出点在 escapeHtml 之后调用 handleInlineCode，避免双重转义
